...
- ตรวจสอบข้อมูลซ้ำก่อนบันทึก — ถ้าวันที่และข้อมูลที่กรอกตรงกับแถวเดิม bot จะหยุดถามยืนยันก่อน
- วันที่ในชีทเป็น `DD/MM/YYYY` เสมอ (เขียนเป็น text ไม่ให้ Google Sheets ตีความตาม locale)
- ระบบสิทธิ์ 3 ระดับ: Super Admin / Authorized User / Unauthorized
- Log การทำงานทุกวันแยกไฟล์ พร้อม log action ละเอียด
- `/setwebsite` และ `/setplatform` ตั้งเว็บ/platform โปรดไว้เป็นปุ่มลัดแรกตอนถูกถาม (ไม่เติมอัตโนมัติ — ต้องเลือกหรือพิมพ์เองทุกครั้ง กันบันทึกผิดเว็บ/ผิด platform)
...
รัน unit test (ครอบคลุม logic ตรวจข้อมูลซ้ำ และการบังคับวันที่เป็น text):
...
หลังกด ✅ ยืนยัน bot จะเช็คก่อนบันทึกว่าข้อมูลนี้ซ้ำกับที่เคยบันทึกไว้หรือไม่ โดยถือว่า "ซ้ำ" เมื่อ
**วันที่ตรงกัน และ field ที่กรอกมาทุกตัวตรงกันทั้งหมด**

- เช็คเฉพาะใน tab ของ **เดือน + เว็บ + platform เดียวกัน** เท่านั้น (อ่าน sheet เพิ่มแค่ 1 request ต่อการยืนยัน)
  ไม่ย้อนเช็คข้ามเดือน/ข้ามปี และถ้าไฟล์เดือนนั้นหรือ tab นั้นยังไม่มี จะข้ามการเช็คไปเลย
- เทียบแบบ trim ช่องว่าง + ไม่สนตัวพิมพ์เล็ก/ใหญ่ ส่วนตัวเลขเทียบเป็นตัวเลข (`1,780.63` = `1780.63`)
- ถ้าเจอซ้ำ bot จะ**ไม่บันทึกทันที** แต่แสดงข้อมูลที่เพิ่งส่งมาเทียบกับแถวเดิม (บอก Row ที่ซ้ำ) พร้อมปุ่ม
  ✅ บันทึกซ้ำจริง / ❌ ยกเลิก
- ทั้งตอนเจอซ้ำและตอน user ตัดสินใจ จะถูกเขียน log ไว้ด้วย action `DATA_DUPLICATE`
- ถ้าเช็คไม่สำเร็จ (Sheets ล่ม/อ่านไม่ได้) bot จะ log error แล้วบันทึกตามปกติ — การเช็คซ้ำไม่บล็อกการบันทึก

### รูปแบบวันที่ในชีท

วันที่ถูกเขียนลงชีทเป็น **plain text** รูปแบบ `DD/MM/YYYY` (เช่น `01/12/2026` = 1 ธันวาคม 2026)
เพราะทุก write path ใช้ `valueInputOption: "USER_ENTERED"` ซึ่ง Google Sheets จะตีความค่าตาม locale
ของไฟล์นั้น — ไฟล์ที่ตั้ง locale เป็น US จะอ่าน `01/12/2026` เป็น 12 มกราคม แทนที่จะเป็น 1 ธันวาคม

ข้อแลกเปลี่ยน: cell วันที่เป็น text จึง sort/คำนวณวันที่ในชีทโดยตรงไม่ได้
(การเทียบวันที่ทั้งหมดของ bot ทำในโค้ดอยู่แล้ว) ใช้กับ**ข้อมูลใหม่จากนี้ไปเท่านั้น** ข้อมูลเก่าไม่ถูกแก้
...
ตั้งเว็บ/platform โปรดไว้เป็นปุ่มลัดแรกตอนถูกถาม (กดปุ่มได้เร็วขึ้น แต่ยังต้องเลือก/พิมพ์เองทุกครั้ง ระบบไม่เติมให้อัตโนมัติ):
